...
class Model_flow(nn.Module):
    ...
    def forward(self, inputs):
        ...
        loss_pack['loss_flow_pixel'] = self.compute_loss_with_mask(diff_fwd, weight_fwd) + \
            self.compute_loss_with_mask(diff_bwd, weight_bwd)
        
        loss_pack['loss_flow_ssim'] = self.compute_loss_ssim(img_pyramid, img_warped_pyramid_from_r, weight_fwd) + \
...
if __name__ == '__main__':
    ...
    model = Model_flow()
    model = model.cuda()
    input = torch.randn((2,3,3264,1920)).cuda()

    loss_pack = model(input)
    print(loss_pack)