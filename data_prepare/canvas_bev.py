...
class Canvas_BEV_heading_right(object):
    ...
    def get_canvas_flows(self, flow_xy):
        flow_xy = np.copy(flow_xy)
        x = flow_xy[:, 0]
        y = flow_xy[:, 1]
        ...
        x = x / (self.canvas_x_range[1] - self.canvas_x_range[0])
        x = x * self.canvas_shape[1]
        x = x.astype(np.int32)

        y = y / (self.canvas_y_range[1] - self.canvas_y_range[0])
    ...
    def draw_canvas_flows(self,
                          ...
                          colors_operand=None):
        """
        Draws the flow from canvas_xy onto self.canvas.

        Args:
            canvas_xy (ndarray): (N, 2) array of *valid* canvas coordinates.
            flow (ndarray): (N, 2) array of flow.
        ...
        for color, (x, y), (f_x, f_y) in zip(colors.tolist(), canvas_xy.tolist(), flow.tolist()):
            # self.canvas = cv2.circle(self.canvas, (x, y), radius, color, 
            #                             -1, lineType=cv2.LINE_AA)
            self.canvas = cv2.line(self.canvas, (x,y), (x+f_x, y+f_y), color, thickness)
