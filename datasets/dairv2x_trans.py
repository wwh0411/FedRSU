...
__all__ = ['Dair_v2x_trans']


class Dair_v2x_trans(data.Dataset):
    """
    Args:
    ...
    def __init__(self,
        ...
        self.root2 = data_root2
        paths1 = ['yizhuang02', 'yizhuang06', 'yizhuang08', 'yizhuang09', 'yizhuang10', 'yizhuang13',
                  'yizhuang16']
        paths2 = ['t1', 't2', 't3', 't4', 't5', 't6', 't7']
        paths = paths1 + paths2
        paths = [x for x in paths for i in range(2)]
        # print(paths)

        self.samples = []
        samples_list = [[] for i in range(28)]

        #paths = [i for i in range(28)]
        for idx, path in enumerate(paths):
            if idx < 14:
                path1 = osp.join(self.root, path)
            else:
                path1 = osp.join(self.root2, path)
            ...
            # print(l)
            if idx % 2 == 0:
                path_list = path_list[:int(l/2)]
            else:
                path_list = path_list[int(l/2):]
            # print(len(path_list))

            for d in path_list:
                samples_list[idx].append(osp.join(tmp_path, d))

        # [1-a,1-b, 2-a,2-b, ..., 1-a-t,1-b-t, 2-a-t,2-b-t]
        if scene == 'all':
            for i in range(28):
                self.samples += samples_list[i]
        elif scene == 'half-all':
            for i in range(14):
                if i % 2 == 0:
                    ...
                    self.samples += samples_list[i+14]
        elif scene == 'origin-all':
            for i in range(14):
                self.samples += samples_list[i]
        elif scene == 'some':
            ...
            #[1-a + 1-a-t, 1-b + 1-b-t, ...]
            self.samples = samples_list[client_id] + samples_list[client_id+ 14]
        elif scene == 'split':
            # n-client = 28
            self.samples = samples_list[client_id]
        elif scene == 'half-origin':
            # n-client = 14
            # [1-a, 1-b, 2-a, 2-b, ...]
            self.samples = samples_list[client_id]
        elif scene == 'half-trans':
            # [1-a-t, 1-b-t, 2-a-t, 2-b-t, ...]
            self.samples = samples_list[client_id + 14]
        elif scene == 'half':
            # [1-a, 1-b-t, 2-a, 2-b-t, ...]
            if client_id %2 == 0:
                self.samples = samples_list[client_id]
            ...
            # [1-a + 1-b, 2, 3, ..., 1-a-t + 1-b-t, 2-t, 3-t, ...]
            self.samples = samples_list[client_id*2] + samples_list[client_id*2+1]
